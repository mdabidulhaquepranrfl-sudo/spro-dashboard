          ...
          <g transform="translate(-27.000000, -15.000000)">
            <g transform="translate(27.000000, 15.000000)">
              <g transform="translate(0.000000, 8.000000)">
                <mask id="mask-2-sidebar" fill="white">
                  <use xlinkHref="#path-1" />
                </mask>
                <use fill="currentColor" xlinkHref="#path-1" />
                <g mask="url(#mask-2-sidebar)">
...
export default function Sidebar() {
  ...
  // Re-initialize Sneat menu after mount so toggles work with Next.js navigation
  useEffect(() => {
    // Menu auto-initializes via Sneat's main.js
  }, [pathname]);
    ...
    >
      ...
      <ul className="menu-inner py-1">
        {SIDEBAR_NAV.map((item) => {
          ...
          const isActive = hasChildren ? isChildActive(item.children) : pathname === item.href;

          return (
            <li
              key={item.id}
              className={`menu-item${isActive ? ' active open' : ''}`}
            >
              {hasChildren ? (
                <>
                  {/* Parent toggle */}
